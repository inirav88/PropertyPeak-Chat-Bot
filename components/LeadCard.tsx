    ...
    <div className={`relative bg-white p-4 rounded-2xl border transition-all group ${
      ...
      <div className="flex justify-between items-start mb-3 ml-7">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 tracking-wider font-mono">{lead.phone}</p>
          </div>
        </div>
        <div className="flex flex-col items-end">
           <span className="text-[10px] font-medium text-slate-400 mb-1">{lead.lastActive}</span>
           <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-tight ${getStatusColor(lead.status)}`}>
      ...
      <div className="grid grid-cols-2 gap-2 mb-2">
        <div className="bg-slate-50 p-2 rounded-xl border border-slate-100">
          <p className="text-[8px] uppercase text-slate-400 font-black tracking-widest">Profile Type</p>
          <p className="text-xs font-bold text-slate-700">{lead.type}</p>
        </div>
        <div className="bg-slate-50 p-2 rounded-xl border border-slate-100">
          <p className="text-[8px] uppercase text-slate-400 font-black tracking-widest">Budget Tier</p>
          <p className="text-xs font-bold text-slate-700">{lead.budget}</p>
        </div>
      </div>

      <div className="mb-4 px-2 py-1.5 bg-slate-50 rounded-xl flex items-center gap-2 border border-slate-100">
        <i className={`${getChannelIcon(lead.leadSource)} text-[10px]`}></i>
        <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Channel: {lead.leadSource || 'Organic Search'}</span>
      </div>
      ...
      <div className="flex items-center justify-between border-t border-slate-100 pt-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] font-black text-emerald-600 uppercase">Conv. Prob: {lead.score}%</span>
        </div>
        <button className="text-[9px] text-emerald-600 font-black hover:underline uppercase tracking-tighter flex items-center gap-1">
          Detailed Activity <i className="fa-solid fa-chevron-right text-[8px]"></i>
        </button>
      </div>
    </div>
  );
